...
void ClientPutInServer( edict_t *pEntity )
{
	...
	else if ( g_pGameRules->IsTeamplay() )//Miagi
	{
		...
		//UTIL_ClientPrintAll( HUD_PRINTNOTIFY, "Team Red" );

		}
		else if ( !stricmp( teaminfo, "blue" ) )
		{
		pPlayer->pev->team = 2;
		pPlayer->pev->iuser4 = pPlayer->pev->team;
		//UTIL_ClientPrintAll( HUD_PRINTNOTIFY, "Team Blue" );
		}
		else
		{
	...
	if ( blueteam < redteam )
	{
		pPlayer->pev->team = 2;
		pPlayer->pev->iuser4 = pPlayer->pev->team;
		g_engfuncs.pfnSetClientKeyValue( pPlayer->entindex(), g_engfuncs.pfnGetInfoKeyBuffer( pPlayer->edict() ), "team", "blue" );
	...
	else if ( blueteam > redteam )
	{
		pPlayer->pev->team = 1;
		pPlayer->pev->iuser4 = pPlayer->pev->team;
		g_engfuncs.pfnSetClientKeyValue( pPlayer->entindex(), g_engfuncs.pfnGetInfoKeyBuffer( pPlayer->edict() ), "team", "red" );
	}
	else
	{
		pPlayer->pev->team = 1;
	...
	// Miagi - Set model to male
		SET_MODEL( pEntity, "models/player/male/male.mdl" );
		g_engfuncs.pfnSetClientKeyValue( pPlayer->entindex(), g_engfuncs.pfnGetInfoKeyBuffer( pPlayer->edict() ), "model", "male" );
...
// make sure the text has content
	//added declaration and changed statement - desNotes
	char *pc; 
...
void ClientCommand( edict_t *pEntity )
{
	...
	else if ( FStrEq(pcmd, "spectate" ) && InArenaMode() == FALSE )
	{
		...
		if ( allow_spectators.value )
		{
			...
			{
			//Miagi set time to be able to hit ready cmd
			pl->m_fNextSuicideTime = gpGlobals->time + 25;
	...
	// Miagi - Allow players to spawn back into game, only if its deathmatch since its not a problem in arena
	//Added a timer, set through spectate cmd
	else if ( FStrEq(pcmd, "ready" ) && InArenaMode() == FALSE )
	{
			...
			if ( pPlayer->IsObserver() )
			{
				...
				if ( pl->m_fNextSuicideTime > gpGlobals->time )
				return;  // prevent respawning too often, look at spectate cmd
...
void ClientUserInfoChanged( edict_t *pEntity, char *infobuffer )
{
	...
	strcpy(rico_model,g_engfuncs.pfnInfoKeyValue( infobuffer, "model" ));

	for( int iCntr = model_index; iCntr < model_len; iCntr++ )
		modelSub[iCntr - model_index] = rico_model[iCntr];

	if ( strcmp(modelSub, "male") != 0 )
		{
			SET_MODEL( pEntity, "models/player/male/male.mdl" );
			g_engfuncs.pfnSetClientKeyValue( ENTINDEX( pEntity ), infobuffer, "model", "male" );
			//UTIL_ClientPrintAll( HUD_PRINTNOTIFY, UTIL_VarArgs( "If strcmp = %i \n", strcmp(modelSub, "male")));
		}
	else
	{
			//UTIL_ClientPrintAll( HUD_PRINTNOTIFY, UTIL_VarArgs( "else strcmp = %i \n", strcmp(modelSub, "male")));
	}

		// Miagi prevent model changing in teamplay
...
//	UTIL_ClientPrintAll( HUD_PRINTNOTIFY, UTIL_VarArgs( "02. checking modelSub = %s rico_model = %s \n", modelSub, rico_model));
//	UTIL_ClientPrintAll( HUD_PRINTNOTIFY, UTIL_VarArgs( "1. checking model= %s modelSub = %s model_len = %i model_index = %i \n", g_engfuncs.pfnInfoKeyValue( infobuffer, "model" ), modelSub, model_len, model_index));
//	UTIL_ClientPrintAll( HUD_PRINTNOTIFY, UTIL_VarArgs( "1.5 checking strcmp = %i \n", strcmp(modelSub, "male")));
	//Set model if no model exists - desNotes 05/13/03
//	if ( (strcmp( "male", g_engfuncs.pfnInfoKeyValue( infobuffer, "model" ) )) ) {
//		SET_MODEL( pEntity, "models/player/male/male.mdl" );
//		g_engfuncs.pfnSetClientKeyValue( ENTINDEX( pEntity ), infobuffer, "model", "male" );
//	}
	//char *test_model = g_engfuncs.pfnInfoKeyValue( infobuffer, "model" );
	//UTIL_ClientPrintAll( HUD_PRINTNOTIFY, UTIL_VarArgs( "2. checking test_model= %s, %i \n", test_model, test_model.find("male")));
//	UTIL_ClientPrintAll( HUD_PRINTNOTIFY, UTIL_VarArgs( "2. checking model= %i, strcmp=  \n", strlen(g_engfuncs.pfnInfoKeyValue( infobuffer, "model" ))));
...
// player pain sounds
	...
	//Enabled precache of female model - desNotes 05/03/03
