...
void CHalfLifeTeamplay :: Think ()
{
	...
	if ( flFragLimit )
	{
		...
		for ( int i = 0; i < num_teams; i++ )
		{
			...
			const int remain = flFragLimit - team_scores[i];
			if ( remain < bestfrags )
			{
				bestfrags = remain;
			}
...
const char *CHalfLifeTeamplay::SetDefaultPlayerTeam( CBasePlayer *pPlayer )
{
	...
	// Miagi changed team from model
	const char *mdls = g_engfuncs.pfnInfoKeyValue( g_engfuncs.pfnGetInfoKeyBuffer( pPlayer->edict() ), "team" );
...
void CHalfLifeTeamplay::ChangePlayerTeam( CBasePlayer *pPlayer, const char *pTeamName, BOOL bKill, BOOL bGib )
{
	...
	{
		damageFlags |= DMG_NEVERGIB;
		//damageFlags |= DMG_ALWAYSGIB; Miagi
	}
	...
	// copy out the team name from the model
	strncpy( pPlayer->m_szTeamName, pTeamName, TEAM_NAME_LENGTH );
//Miagi set team so no tk's can happen
	if ( !stricmp( pPlayer->m_szTeamName, "red" ) )
	{ 
		pPlayer->pev->team = 1;
		//UTIL_ClientPrintAll( HUD_PRINTNOTIFY, "Team Red" );

	}
	else if ( !stricmp( pPlayer->m_szTeamName, "blue" ) )
...
void CHalfLifeTeamplay::ClientUserInfoChanged( CBasePlayer *pPlayer, char *infobuffer )
{
	...
	char *mdls = g_engfuncs.pfnInfoKeyValue( infobuffer, "team" );
...
const char *CHalfLifeTeamplay::TeamWithFewestPlayers()
{
	...
	for ( i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( plr )
		{
			const int team = GetTeamIndex( plr->TeamID() );
			if ( team >= 0 )
				teamCount[team] ++;
		}
	}
