...
void CWorld :: Precache()
{
	...
	{
		g_pArenaList[i] = GetClassPtr( ( CDiscArena *)nullptr);
...
void CWorld :: KeyValue( KeyValueData *pkvd )
{
	...
	/*else if ( FStrEq(pkvd->szKeyName, "no_arena") )
	{
		if ( atoi(pkvd->szValue) )
		{
			if ( atoi(pkvd->szValue) == 2 ) // Wha? - Teamplay
			{
				g_iTeamplay = 1; //Miagi - Teamplay
				
			}
			else
			{
				g_iTeamplay = 0; //Miagi - Not teamplay
			}// End Wha?
	...
	else if ( FStrEq(pkvd->szKeyName, "no_arena") ) // Begin Wha? - Teamplay
	{
		...
		case 1:
			...
			g_iTeamplay = 0;
			break;
		default: // In case they actually DO specify no_arena as 0 (or give it a weird value)
			m_iArenaOff = FALSE;
			g_iTeamplay = 0;
			break;
		}
	...
	if ( !m_iArenaOff ) // Begin Wha? - Teamplay - Hack to detect arena maps, since zero-valued keys aren't written to a map
	{ 
		g_iTeamplay = 0;
	} // End Wha?
